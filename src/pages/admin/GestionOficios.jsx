    ...
    const loadTrades = async () => {
      try {
        ...
        const normalized = data.map((oficio) => ({
          ...
          name: oficio.nombre,
          category:
            oficio.categoria_nombre ||
            oficio.categoria ||
            'Sin categoría',
          activeCount:
            oficio.profesionales_activos ||
            oficio.activeCount ||
            0,
          icon:
            oficio.icono_url ||
      ...
      const data = await createTrade(
        {
          ...
          icono_url: newIcon,
          categoria: newCategory.trim() || null,
        },
        token
      )
    ...
    <div className="p-6 space-y-6 bg-slate-100 min-h-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-0.5">
            Administra las profesiones y oficios disponibles
            para los usuarios en la plataforma.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <article className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm h-fit">
          ...
          <h2 className="font-bold text-slate-950 text-lg mb-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md font-mono">
                  Seleccionado: {newIcon}
                </span>
              </div>

              <div className="grid grid-cols-8 gap-1.5 p-2 bg-slate-50 rounded-2xl border border-slate-200 max-h-80 overflow-y-auto">

                {EMOJI_KEYBOARD.map((emoji, index) => (
                  <button
                    key={`${emoji}-${index}`}
                    type="button"
          ...
          <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Total de {trades.length} oficios en el sistema
              </p>
            </div>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td
                      ...
                      className="px-6 py-8 text-center text-slate-400 text-sm"
                    >
                      No se encontraron oficios que coincidan
                      con la búsqueda.
                    </td>
                  </tr>
