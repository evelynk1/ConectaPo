...
export default function Detalle() {
  ...
  // 1. AHORA SOLO SELECCIONA VISUALMENTE (No va a la BD todavía)

  const handleSeleccionarBloque = (bloque) => {

    if (!user) {

      alert("Para agendar, primero debes iniciar sesión en ConectaPo.")
  ...
  // 2. LA RESERVA REAL SE HACE AL TOCAR EL BOTÓN DE WHATSAPP

  const handleContactarWhatsApp = (e) => {

    e.preventDefault();

    // Abrimos WhatsApp inmediatamente para que el navegador no bloquee el popup

    window.open(whatsappUrl, '_blank');

    // Si hay un bloque seleccionado, lo bloqueamos en la Base de Datos

    if (bloqueSeleccionado && bloqueSeleccionado.estado !== 'RESERVADO') {

      setIsReserving(true);

      reservarBloqueCliente(bloqueSeleccionado.id, token)

        .then(() => {

          setBloques(prev => prev.map(b => b.id === bloqueSeleccionado.id ? { ...b, estado: 'RESERVADO' } : b));

          setBloqueSeleccionado(prev => ({...prev, estado: 'RESERVADO'}));

        })

        .catch(err => {

          alert(err.message || 'Error interno: No se pudo asegurar la hora en la plataforma.');
  ...
  const handleCalificar = () => {
    ...
    if (!user) {
      ...
      alert('Para calificar este servicio, primero debes iniciar sesión.')
    ...
    if (ratingSeleccionado === 0) {
      ...
      alert('Selecciona una cantidad de estrellas para calificar.')
  ...
  const isOwner = user?.id === s.usuario_id // 👑 Verificamos si el que mira es el dueño
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
              ...
              <h2 className="font-semibold text-slate-900 mb-3">Califica este servicio</h2>
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    {[1, 2, 3, 4, 5].map((estrella) => (
                      ...
                      <button
                        ...
                        onClick={() => setRatingSeleccionado(estrella)}
              ...
              <div className="p-5 border-b border-slate-100">
                ...
                {isOwner ? (
                  ...
                  <div className="p-4 rounded-xl bg-blue-50 border border-blue-100 text-center">
                    ...
                    <p className="text-blue-800 font-bold text-sm mb-1">Tu Publicación</p>

                    <p className="text-blue-600 text-xs">Los clientes verán tus horarios disponibles aquí.</p>

                  </div>

                ) : bloqueSeleccionado?.estado === 'RESERVADO' ? (
                  ...
                  <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 text-center">
                    ...
                    <p className="text-emerald-600 text-xs mt-2 font-medium">Acabas de coordinar con {s.name}.</p>
                  ...
                  <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                    ...
                    {bloquesDisponibles.map(bloque => {
                      ...
                      const isSelected = bloqueSeleccionado?.id === bloque.id;
                      ...
                      return (
                        ...
                        <button
                          ...
                          className={`flex flex-col items-center justify-center p-2 rounded-lg border transition-all focus:outline-none group
                        ...
                        >
                          ...
                          <span className={`text-[10px] font-medium uppercase tracking-wider ${isSelected ? 'text-blue-700' : 'text-slate-500 group-hover:text-blue-600'}`}>

                            {f.toLocaleDateString('es-CL', { weekday: 'short', day: '2-digit', month: 'short' })}

                          </span>

                          <span className={`text-sm font-bold mt-0.5 ${isSelected ? 'text-blue-800' : 'text-slate-800 group-hover:text-blue-700'}`}>
              ...
              <div className="p-5 space-y-3">
                ...
                {isOwner ? (
                  ...
                  <button 
                    ...
                    className="w-full py-3.5 rounded-xl font-semibold text-slate-400 text-sm bg-slate-100 cursor-not-allowed"
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center gap-2.5 w-full py-3.5 rounded-xl font-semibold text-white text-sm transition-all hover:opacity-90 hover:shadow-lg bg-[#25D366] disabled:opacity-70 disabled:cursor-wait"
                  ...
                  >
                    ...
                    {isReserving ? 'Confirmando...' : 'Contactar por WhatsApp'}
